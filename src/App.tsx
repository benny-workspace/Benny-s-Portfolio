...
export default function App() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    notifyToast("Inquiry registered! Sending email notification...");

    // Send email via our full-stack Express API in background
    try {
      ...
      if (response.ok) {
        notifyToast("Email sent successfully to Benny via Resend!", "success");
      } else {
        const errorData = await response.json();
        console.error("Backend failed to send email:", errorData);
        notifyToast("Inquiry saved locally, but email sending failed.", "error");
      }
    } catch (err) {
      console.error("Network error sending email:", err);
      notifyToast("Inquiry saved locally. Email sending failed (network error).", "error");
    }
  };
                    ...
                    <div className="lg:col-span-7 space-y-6">
                      ...
                      <div className="pt-4 flex gap-4">
                        ...
                        <button
                          ...
                          className="roll-btn border border-coal-900/15 hover:border-coal-950 text-coal-700 hover:text-bone-50 hover:bg-coal-950 text-xs font-semibold px-5 py-2.5 rounded-full transition-all duration-300 cursor-pointer overflow-hidden active:scale-95"
                        >
                          {renderRollingButtonText("Queries / Staying Updated")}
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            >
              ...
              <section className="py-16 sm:py-20 px-4 sm:px-8 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <GlowBorderCard glowColor="orange" theme={isDarkMode ? "dark" : "light"} className={`${isDarkMode ? "bg-coal-950 text-bone-50" : "bg-bone-100/50 text-coal-950"} px-6 pt-14 pb-6 sm:px-8 sm:pt-16 sm:pb-8 flex flex-col justify-between group shadow-md`}>
                    {/* RECOMMENDED PLACARD */}
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-start">
                        <span className={`p-2 rounded-lg ${isDarkMode ? "bg-white/10 text-white" : "bg-coal-950/5 text-coal-950"}`}>
                          <Phone className={`w-5 h-5 animate-pulse ${isDarkMode ? "text-amber-300" : "text-amber-600"}`} />
                        </span>
                        <div className="text-right flex flex-col items-end">
                          <div className="flex items-baseline gap-1">
                            <span className={`font-display font-bold text-2xl block ${isDarkMode ? "text-white" : "text-coal-950"}`}>$2,000</span>
                            <span className={`font-mono text-[8px] uppercase ${isDarkMode ? "text-zinc-400" : "text-coal-600"}`}>SETUP</span>
                          </div>
                          <div className="flex items-baseline gap-1 mt-0.5">
                            <span className={`font-display font-bold text-lg block ${isDarkMode ? "text-amber-400" : "text-amber-600"}`}>$299</span>
                            <span className={`font-mono text-[8px] uppercase ${isDarkMode ? "text-zinc-400" : "text-coal-600"}`}>/ MO ONGOING</span>
                          </div>
                        </div>
                      </div>
                      <h3 className={`font-display font-bold text-xl ${isDarkMode ? "text-white" : "text-coal-950"}`}>24/7 AI Call Reception & Automated Booking</h3>
                      <p className={`text-xs leading-relaxed font-light ${isDarkMode ? "text-zinc-300" : "text-coal-600"}`}>
                      ...
                      <ul className={`text-xs space-y-2 pt-2 border-t ${isDarkMode ? "text-zinc-400 border-white/10" : "text-coal-600 border-coal-900/5"}`}>
                        <li className="flex items-center gap-1.5">
                          <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${isDarkMode ? "bg-amber-300" : "bg-amber-500"}`} />
                          <span>24/7 live call answering & intelligent routing</span>
                        </li>
                        <li className="flex items-center gap-1.5">
                          <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${isDarkMode ? "bg-zinc-500" : "bg-coal-950"}`} />
                    ...
                    <button
                      ...
                      className={`roll-btn w-full mt-6 py-3.5 transition-all duration-300 rounded-xl shadow-sm hover:shadow-md active:scale-95 cursor-pointer overflow-hidden border ${isDarkMode ? "bg-bone-50 hover:bg-zinc-100 text-coal-950 border-white/10" : "bg-coal-950 hover:bg-coal-800 text-bone-50 border-coal-950/10"}`}
                    >
